...
def spawn_mole(instance, plant, simulator):
    ...
    mole_angles = []
    mole_angles.append(np.random.uniform(np.pi / 12, np.pi * 5 / 12))
    mole_angles.append(np.random.uniform(np.pi * 7 / 12, np.pi * 11 / 12))
    mole_angles.append(np.random.uniform(np.pi * 13 / 12, np.pi * 17 / 12))
    mole_angles.append(np.random.uniform(np.pi * 19 / 12, np.pi * 23 / 12))
    mole_angle = random.choice(mole_angles)
...
def despawn_closest_mole(plant, simulator, ee_position):
    closest_mole = np.random.randint(0, MOLE_COUNT)
    closest_dist = np.inf
    for i in range(MOLE_COUNT):
        mole_instance = f"sphere_mole_{i+1:02d}"
        mole_model_instance = plant.GetModelInstanceByName(mole_instance)
        ...
        context = plant.GetMyMutableContextFromRoot(simulator.get_mutable_context())
        mole_position = plant.EvalBodyPoseInWorld(context, mole_body).translation()
        dist = np.linalg.norm(mole_position[:2] - ee_position[:2])
        if dist < closest_dist:
            closest_dist = dist
            closest_mole = i
    despawn_mole(closest_mole, plant, simulator)
    spawn_mole(closest_mole, plant, simulator)
